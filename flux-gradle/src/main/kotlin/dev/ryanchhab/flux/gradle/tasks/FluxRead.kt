 ...
 * `dev.ryanchhab.flux.LIVE_READ`, pulls back the robot's answer, and prints a table of source value vs. robot
    ...
    fun read() {
        ...
        try {
            ...
            val broadcastResult = exec.exec {
                commandLine = listOf(adb, "shell", "am", "broadcast", "-a", "dev.ryanchhab.flux.LIVE_READ")
            ...
            if (code == null) {
                throw GradleException(
                    RobotReachability.explain(exec, adb, "dev.ryanchhab.flux.LIVE_READ", stdout),
                )
            }
            ...
            when (code) {
                ...
                else -> throw GradleException("Flux: unexpected result code $code from dev.ryanchhab.flux.LIVE_READ.")
            }
        } finally {
            localRequest.delete()
            localResult.delete()
