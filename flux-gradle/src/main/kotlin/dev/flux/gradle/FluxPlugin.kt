...
/**
 * `dev.flux.load` — see CONTRACT.md for the frozen coordinates/task names this plugin must match
 * exactly, and architecture.md §9 for the zero-config goal this whole plugin is built around:
 * `plugins { id("dev.flux.load") }` plus `./gradlew fluxDeploy` should be the entire user-facing
 * surface.
 *
...
abstract class FluxPlugin @Inject constructor(
        ...
        project.afterEvaluate {
            if (!wired) {
                throw GradleException(
                    "dev.flux.load must be applied to an Android module.\n" +
                        "  Reason: no com.android.library or com.android.application plugin was " +
                        "found on ${project.path}.\n" +
                        "  Fix: apply dev.flux.load to your TeamCode module (the one with " +
    ...
    private fun wireTasks(
        ...
        val fluxReload = project.tasks.register<FluxReload>("fluxReload") {
            group = "flux"
            description = "adb shell am broadcast -a dev.flux.RELOAD, parse result code (CONTRACT.md)."
            dependsOn(fluxPush)
            adbPath.set(adbPathProvider)
        ...
        val fluxTune = project.tasks.register<FluxTune>("fluxTune") {
            group = "flux"
            description = "Push live_values.json + adb shell am broadcast -a dev.flux.LIVE_TUNE (CONTRACT.md Amendment 3)."
