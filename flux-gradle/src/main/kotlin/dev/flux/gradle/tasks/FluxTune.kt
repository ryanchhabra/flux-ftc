 ...
 * `/sdcard/FIRST/flux/live_values.json`, then `adb shell am broadcast -a dev.flux.LIVE_TUNE` and
 * parses `result=N`.
 *
 ...
 * needs the changed-field list [dev.flux.gradle.LiveTuneDetector] already produced and persisted
 * during `fluxAssemble` (see [dev.flux.gradle.LiveTuneOutcome]'s class doc — classify once,
 * re-read many times, same pattern as [dev.flux.gradle.TierOutcome]).
 *
 * Reuses [dev.flux.gradle.AdbLocator] (via the `adbPath` input, resolved once in FluxPlugin) and
    ...
    fun tune() {
        ...
        try {
            ...
            val broadcastResult = exec.exec {
                commandLine = listOf(adb, "shell", "am", "broadcast", "-a", "dev.flux.LIVE_TUNE")
            ...
            when (code) {
                ...
                3 -> throw GradleException(
                    ...
                    "Flux: no result code from the robot (raw adb output: \"${stdout.trim()}\").\n" +
                        ...
                        "dev.flux.LIVE_TUNE (CONTRACT.md Amendment 3 -- the robot-side receiver may " +
                        "not be built yet).\n" +
